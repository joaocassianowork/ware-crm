const Projects = (() => {
  ...
  function render(){
    ...
    el.innerHTML=Store.projects.map(proj=>{
      ...
      const done=ptasks.filter(t=>t.done).length;const total=ptasks.length;
  ...
  async function openDetail(id){
    ...
    const svcs=[...new Set(ptasks.map(t=>t.service))];
    UI.set('dpProjChecklist',svcs.map(svc=>{
      const st=ptasks.filter(t=>t.service===svc);
      return `<div class="chk-group"><div class="chk-svc-lbl">${svc}</div>${st.map(t=>`<div class="chk-item"><div class="chk-box ${t.done?'done':''}" onclick="Projects.toggleTask('${t.id}',${!t.done})"></div><div class="chk-label ${t.done?'done':''}">${t.task}</div>${t.done_at?`<div class="chk-date">${Utils.timeAgo(t.done_at)}</div>`:''}</div>`).join('')}</div>`;
    }).join(''));
    UI.openDP('dpProject');
  }
  async function toggleTask(id,done){
    await sb.from('project_tasks').update({done,done_at:done?new Date().toISOString():null}).eq('id',id);
  ...
  return{render,openDetail,toggleTask,updateChecklist,save,openModal};
})();
