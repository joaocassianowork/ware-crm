const Finance = (() => {
  ...
  function dueDay10(key){return key+'-10';}
  function contractCoversMonth(client,key){
    const start=(client.contract_start||key+'-01').slice(0,7);
    const end=(client.contract_end||key+'-28').slice(0,7);
  ...
  async function ensureMonthlyPayments(){
    ...
    const rows=Store.clients
      .filter(c=>c.status==='ativo'&&c.monthly_value&&contractCoversMonth(c,key))
      .filter(c=>!Store.payments.some(p=>p.client_id===c.id&&(p.month||'').slice(0,7)===key))
      .map(c=>({client_id:c.id,month:monthStart(key),amount:c.monthly_value,due_date:dueDay10(key),status:'pendente'}));
    if(rows.length){
      const{data,error}=await sb.from('payments').insert(rows).select('*');
      if(!error&&data)Store.payments=[...Store.payments,...data];
    }
    generating=false;
  }
  function render(){
  ...
  function openModal(){
    ...
    document.getElementById('pDue').value=dueDay10(monthKey());document.getElementById('pAmount').value='';document.getElementById('pStatus').value='pendente';
    UI.openModal('moPayment');
  }
  return{render,markPaid,save,openModal,deletePayment,ensureMonthlyPayments};
})();
