const Clients = (() => {
  ...
  async function deleteCurrent(){
    ...
    if(!ok)return;
    async function ignoreDelete(query){try{await query;}catch(e){}}
    const{data:projects}=await sb.from('projects').select('id').eq('client_id',curId);
    const projectIds=[...(Store.projects.filter(p=>p.client_id===curId).map(p=>p.id)),...((projects||[]).map(p=>p.id))].filter((v,i,a)=>v&&a.indexOf(v)===i);
    if(projectIds.length)await ignoreDelete(sb.from('project_tasks').delete().in('project_id',projectIds));
    if(projectIds.length)await ignoreDelete(sb.from('projects').delete().in('id',projectIds));
    const tables=['payments','client_timeline','onboardings','approvals','reports','meetings','satisfaction_surveys','client_accesses','client_milestones','client_checkins','campaign_metrics','client_notes'];
    for(const table of tables)await ignoreDelete(sb.from(table).delete().eq('client_id',curId));
    const{error}=await sb.from('clients').delete().eq('id',curId);
    if(error){alert('Nao foi possivel excluir o cliente.\n\nSupabase: '+(error.message||'verifique permissoes/RLS ou chaves estrangeiras.'));return;}
